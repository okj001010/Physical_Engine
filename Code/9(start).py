...
class Palette: # 팔레트 class
    ...
    def start(self, mousex, mousey, clicked):
...
def PlaceAllObj(mousex, mousey, clicked, r_clicked):
    ...
    if not anyselected: # 클릭된게 하나도 없었던 상황에서
        for i in range(len(objarr)):
            if objarr[objorder.index(i)].click(mousex, mousey, clicked, False, False): # 클릭되면
                j = objorder.index(i)
                changeObjorder(j)
                ...
                break
            if objarr[objorder.index(i)].click(mousex, mousey, False, r_clicked, False):
